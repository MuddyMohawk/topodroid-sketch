 ...
 * @date jun 2026
 *
 * @brief TopoDroid Sketch brush-style rendering helpers
 * --------------------------------------------------------
...
class SketchBrushRenderer
{
  ...
  static Paint linePaint( Paint source, SketchBrushStyle style, float fixed_pattern_scale )
  {
    return paint( source, style, fixed_pattern_scale );
  }
  ...
  private static Paint paint( Paint source, SketchBrushStyle style, float width_scale )
  {
    ...
    if ( style.hasWeight() ) {
      paint.setStrokeWidth( style.weightOr( SketchBrushStyle.DEFAULT_WEIGHT_STANDARD )
                            * TDSetting.mLineThickness * normalizedPositive( width_scale ) );
    }
    return paint;
  }
