...
class DrawingXvi
{
  ...
  static void writeXvi( BufferedWriter out, TDNum num, DrawingCommandManager plot, long type )
  {
    ...
    // TDLog.v("XVI origin " + origin.name + " " + origin.e + " " + origin.s + " " + origin.h + " " + origin.v + " type " + type );
	...
	out.write("}\n\n");
      ...
      for ( ICanvasCommand cmd : plot.getCommands() ) {
	    ...
	    // TDLog.v( " yoff " + yoff + " cy " + point.cy + " yy " + yy );
  ...
  {
    // TDLog.v("XVI " + st.getName() + " " + st.cx + " " + st.cy + " off " + xoff + " " + yoff);
  ...
  static private void tdrToXvi( PrintWriter pw, String scrapfile, float dx, float dy, float xoff, float yoff )
  {
    ...
    // TDLog.v( "trd to xvi. scrap file " + scrapfile + " shift " + dx + " " + dy + " offset " + xoff + " " + yoff );
    try {
      ...
      // TDLog.v( "tdr to xvi delta " + dx + " " + dy + " Offset " + xoff + " " + yoff );
