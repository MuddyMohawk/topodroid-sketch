...
# Runs RenderIdentityInstrumentedTest: renders the Demo fixture off-screen
# under fixed transforms/display variants and reports SHA-256 pixel hashes
# (render_hashes.txt + render_*.png in tmp-test-artifacts).
# Workflow: run on an unmodified tree, stash tmp-test-artifacts as baseline,
# re-run after a renderer change, byte-compare render_hashes.txt.
...
$RenderTests = @(
  @{
    Name = "Render identity hashes"
    Class = "com.topodroid.TDX.RenderIdentityInstrumentedTest#renderDemoFixture_reportsPixelHashes"
    Estimate = 240
    Timeout = 600
    IdleTimeout = 300
  }
...
try {
  ...
  Clear-LocalArtifactDirectory -Path $ArtifactsLocal -Description "render identity artifacts"
